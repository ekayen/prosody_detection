...
import pandas as pd
import yaml
from utils import load_data,BatchWrapper,to_ints,load_vectors,make_batches,load_libri_data,plot_results,make_nonseq_batches
from evaluate import evaluate,nonseq_evaluate
import numpy as np
import sys
...
class BiLSTM(nn.Module):
    ...
    def forward(self,sent,hidden):
        ...
        last_timestep = torch.mean(lstm_out,0)
        tag_space = self.hidden2tag(last_timestep)
        tag_scores = self.sigmoid(tag_space)
        if print_dims:
            print('sent.shape',sent.shape)
            print('embeds.shape', embeds.shape)
            print('lstm_out.shape', lstm_out.shape)
            print('last_timestep.shape',last_timestep.shape)
...
'''
Y_train_str = [y.tolist()[-1] for y in Y_train]
Y_dev_str = [y.tolist()[-1] for y in Y_dev]
...
if not datasource == 'LIBRI':
    train_acc = nonseq_evaluate(X_train, Y_train_str, model, device)
    train_accs.append(train_acc)
else:  # Don't do train acc every time for bigger datasets than SWBDNXT
    train_accs.append(0)
dev_acc = nonseq_evaluate(X_dev, Y_dev_str, model, device)
dev_accs.append(dev_acc)
timesteps.append(0)

for epoch in range(num_epochs):
    # BATCH DATA
    X_train_batches, Y_train_batches = make_nonseq_batches(X_train, Y_train, batch_size, device)
...
nonseq_evaluate(X_train,Y_train_str,model,device)


print('After training, dev: ')
nonseq_evaluate(X_dev, Y_dev_str,model,device)#,True)

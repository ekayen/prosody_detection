...
def evaluate_seg(cfg,dataset,dataloader_params,model,device,recurrent=True,tok_level_pred=False,noisy=True,text_only=False,
    ...
    with torch.no_grad():
        counter = 0
        for iden, frames, pause,dur, labels, toktimes, tokints in dataloader:
            y = labels
            ...
            frames,pause,dur,y,tokints = frames.to(device),pause.to(device),dur.to(device),y.to(device),tokints.to(device)
            ...
            if print_predictions:
                ...
                sents = [np.trim_zeros(np.array(tokints[i:i+1].flatten().cpu()),'b').tolist() for i in range(curr_bat_size)]
                ...
                print_lines.extend([(sents[i],lbls[i],preds[i],iden[i]) for i in range(len(sents))])
